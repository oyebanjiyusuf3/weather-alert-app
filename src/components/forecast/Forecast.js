  ...
  return (
    <>
        ...
        <div className="week-container">
          ...
          <hr />
          <h3 className="text-character">Forecast wind information for the next 7 days</h3>
      
       
        <ul className="week-list">
        {data.list.splice(0, 7).map((item, idx) => (
        <li className="active" key={idx}><img className="day-icon" src={`icons/${item.weather[0].icon}.png`} data-feather="sun"/><span className="day-name">{forecastDays[idx]}</span><span className="day-temp">{item.wind.speed} mph</span></li>
        ))}
