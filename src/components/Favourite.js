...
const Favourite = ({loadOptions}) => {
  ...
  <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
        <img className="day-icon" src={'icons/' + localStorage.getItem('icon')+'.png'}  data-feather="sun"/>
        <div className="wind-deg"> <span className="title">WIND SPEED </span><span className="value">{localStorage.getItem('wind')} mph </span>
          <div className="clear"></div>
        </div>
        <div className="wind-deg"> <span className="title">WIND DEGREE</span><span className="value">{localStorage.getItem('degree')}° </span>
          <div className="clear"></div>
        </div>
        <div className="temperatur"> <span className="title">TEMPERATURE</span><span className="value">{Math.round(localStorage.getItem('temp'))}°C</span>
          <div className="clear"></div>
        </div>
       
        <div className="wind"> <span className="title">FEELS LIKE</span><span className="value">{Math.round(localStorage.getItem('feels_like'))}°C</span>
          <div className="clear"></div>
        </div>
        <div className="humidity"> <span className="title">HUMIDITY</span><span className="value">{localStorage.getItem('humidity')} %</span>
          <div className="clear"></div>
        </div>
        <div className="humidity"> <span className="title">PRESSURE</span><span className="value">{localStorage.getItem('pressure')} hPa</span>
          <div className="clear"></div>
        </div>
  ...
  </li>   : <h4 className="text-character">No favourite data previously added </h4> 
  }
